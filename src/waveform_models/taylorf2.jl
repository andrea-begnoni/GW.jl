...
a function of frequency, given the events parameters.  
    ...
    Pol(TaylorF2(), mc, eta, chi1, chi2, dL)
...
#### Example:
    ...
    pol = Pol(TaylorF2(), f, 10., 0.20, 0.1, 0.2, 10.)
    hp = pol["plus"]
    hc = pol["cross"]
"""
function Pol(
    ...
    phi = Phi(
        ...
        is_tidal = is_tidal,
        use_QuadMonTid = use_QuadMonTid,
        is_eccentric = is_eccentric,
        phiref_vlso = phiref_vlso,
        use_3p5PN_SpinHO = use_3p5PN_SpinHO,
        fRef_ecc = fRef_ecc,
    )
